...
router.get('/item/:slug', async (req, res) => {
  try {
    ...
    res.status(400).json({
      error: 'Tu solicitud no pudo ser procesada. Inténtalo de nuevo.'
    });
  }
});
...
router.get('/list/brand/:slug', async (req, res) => {
  try {
    ...
    if (!brand) {
      return res.status(404).json({
        message: `No se puede encontrar la marca con el slug ${slug}.`
      });
    }
    ...
    try {
     ...
     // console.log(req.body)
      ...
      if (!description || !name) {
        return res
          .status(400)
          .json({ error: 'Debe ingresar descripción y nombre.' });
      }
      ...
      if (!price) {
        return res.status(400).json({ error: 'Debe introducir un precio.' });
      }
